...
rho_o = P_o / (cs ** 2)
def f_lane_emden(y, t):
    theta, omega = y
    dydt = [omega, -2.0*omega/t - np.exp(theta)]
    return dydt
# ----Solve Lane-Emden----#
y_init = [0, 0]  # initial conditions
t = sp.linspace(0.0001, 7, 5000)  # create array of values. Equidistant integration steps.
...
plt.rc('font', **font)
plt.figure(1)
# plt.plot(t, rho_frac, color='LightSeaGreen', linewidth=2, label='$\psi$')
plt.plot(t, dpsi, color='Plum', linewidth=2, label='$\\rho$/$\\rho_c$')
